...
public class ProjectHeadController {
    ...
    public ResponseEntity<ApiResponse<Void>> deleteUser(@AuthenticationPrincipal UserPrincipal userPrincipal,
                                                       @PathVariable Long id) {
        projectHeadService.deleteUser(id, userPrincipal.getId(), userPrincipal.getUsername());
        return ResponseEntity.ok(ApiResponse.success("User account removed successfully"));
    }

    @DeleteMapping("/groups/{id}")
    public ResponseEntity<ApiResponse<Void>> deleteGroup(@AuthenticationPrincipal UserPrincipal userPrincipal,
                                                        @PathVariable Long id) {
        projectHeadService.deleteGroup(id, userPrincipal.getId(), userPrincipal.getUsername());
        return ResponseEntity.ok(ApiResponse.success("Project group deleted successfully"));
    }

    @DeleteMapping("/groups/{groupId}/members/{studentId}")
    public ResponseEntity<ApiResponse<Void>> removeStudentFromGroup(@AuthenticationPrincipal UserPrincipal userPrincipal,
                                                                   @PathVariable Long groupId,
                                                                   @PathVariable Long studentId) {
        projectHeadService.removeStudentFromGroup(studentId, userPrincipal.getId(), userPrincipal.getUsername());
        return ResponseEntity.ok(ApiResponse.success("Student removed from group successfully"));
    }

    @PostMapping("/groups/{groupId}/members")
    public ResponseEntity<ApiResponse<Void>> addStudentToGroup(@AuthenticationPrincipal UserPrincipal userPrincipal,
                                                              @PathVariable Long groupId,
                                                              @RequestBody Map<String, Object> payload) {
        Long studentId = Long.valueOf(payload.get("studentId").toString());
        boolean isLeader = payload.get("isLeader") != null && Boolean.parseBoolean(payload.get("isLeader").toString());
        projectHeadService.addStudentToGroup(groupId, studentId, isLeader, userPrincipal.getId(), userPrincipal.getUsername());
        return ResponseEntity.ok(ApiResponse.success("Student assigned to group successfully"));
    }
